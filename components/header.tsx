...
export function Header({ language, onLanguageChange, copy }: HeaderProps) {
  const menus = language === "zh-CN" ? {
    ...
    pdf: ["PDF 转 Word", "Word 转 PDF", "PDF 在线编辑", "PDF 合并", "PDF 压缩"],
    ...
    pdf: ["PDF to Word", "Word to PDF", "PDF Editor", "Merge PDF", "Compress PDF"],
  ...
  return (
    <header className="site-header section-shell">
      ...
      <nav aria-label="Main navigation">
        ...
        <div className="nav-menu"><a href="#tools">{copy.navigation.pdf}</a><div className="tool-menu"><a href={`/tools/pdf-to-word?lang=${language}`}>{menus.pdf[0]}</a><a href={`/tools/word-to-pdf?lang=${language}`}>{menus.pdf[1]}</a><a href={`/tools/pdf-editor?lang=${language}`}>{menus.pdf[2]}</a><a href="#tools">{menus.pdf[3]}</a><a href="#tools">{menus.pdf[4]}</a></div></div>
