...
export function ToolWorkspace({ tool }: { tool: Tool }) {
  const [input, setInput] = useState(""); const [output, setOutput] = useState(""); const [status, setStatus] = useState<string | null>(null); const stats = useMemo(() => getTextStats(input), [input]);
  ...
  return <section className="workspace"><div className="workspace-top"><label htmlFor="tool-input">Your text</label><div className="action-row"><button type="button" className="text-button" onClick={() => navigator.clipboard.readText().then(setInput).catch(() => setStatus("Paste is unavailable in this browser."))}>Paste</button><button type="button" className="text-button" onClick={clear}>Clear</button></div></div><textarea id="tool-input" value={input} onChange={(event) => setInput(event.target.value)} placeholder={tool.slug.startsWith("json") ? '{"hello":"world"}' : "Type or paste here..."} spellCheck={false} />
    ...
    {!isCounter && <><div className="workspace-top output-label"><label htmlFor="tool-output">Result</label><button type="button" className="text-button" onClick={() => copy(output)}>Copy result</button></div><textarea id="tool-output" value={output} placeholder="Your result will appear here..." readOnly spellCheck={false} /></>}
    <p className={`status ${status?.startsWith("Invalid") || status?.startsWith("Please") ? "error" : ""}`} aria-live="polite">{status || "Processed locally in your browser."}</p>
  </section>;
}
